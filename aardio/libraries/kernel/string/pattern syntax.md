...
\c | 控制字符 | \C | 不是控制字符的字符
\d | 数字 | \D | 不是数字的字符
\i | 是否ASCII字符( 字节码 < 0x80 ) | \I | 不是ASCII字符( 字节码 >= 0x80 )
\l | 小写字母 | \L | 不是小写字母的字符
\p | 标点字符 | \P | 不是标点的字符
\s | 空白符 | \S | 不是空白符的字符
\u | 大写字母 | \U | 不是大写字母的字符
\w | 字母和数字、以及下划线 | \W | 不是字母和数字、并且不是下划线的字符
\x | 十六进制数字 | \X | 不是十六进制数字的字符
\n | 换行符 | \N | 不是换行符
\r | 回车符  | \R | 不是回车符
\f | 换页符 '\x0c' | \F | 不是换页符
\v | 匹配一个垂直制表符。等价于 '\x0b'  | \V | 不是垂直制表符
\z | 表示'\0', 注意在模式串是纯文本，<br/>'\0'表示模式串结束，所以需要用"\z"表示'\0' . | \Z | 不是'\0'的字符
...
例如 <^hello> 匹配所有不是hello并且`等长`的字符.如果是在串的结束,则允许至少匹配一个字符.
...
//匹配结尾不能为中英文标点的文本，返回值应当为null
abc =  string.match( "123ABC。",'.+<.\\P>&<^\xA1\xA1-\xFF>$')

//匹配结尾不能为中英文标点的文本，返回值应当为"456cde"
cde =  string.match( "456cde",'.+<.\\P>&<^\xA1\xA1-\xFF>$')
io.print( abc ,cde)
